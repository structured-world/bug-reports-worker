/**
 * Cloudflare Worker: Bug Reports API for all sw.foundation docs sites
 *
 * Handles POST /api/report-bug - creates GitHub issues from docs feedback widget.
 * Supports multiple sites with per-site repository configuration.
 *
...
const SITES: Record<string, SiteConfig> = {
  "gitlab-mcp.sw.foundation": {
    repo: "structured-world/gitlab-mcp",
    allowedOrigins: [
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "OPTIONS") {
      return handleOptions(request, siteConfig.allowedOrigins);
    }

    if (request.method === "POST") {
      return handlePost(request, env, siteConfig);
    }
...
async function handlePost(
  ...
  const origin = request.headers.get("Origin") || "";
  const headers = {
    ...corsHeaders(origin, siteConfig.allowedOrigins),
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, issue: issue.number }),
      {
        status: 201,
        headers,
      },
...
function validateReport(
  ...
  const page =
    typeof raw.page === "string" ? raw.page.trim().slice(0, 200) : "/unknown";
  const expected =
    typeof raw.expected === "string"
      ? raw.expected.trim().slice(0, 2000)
      : undefined;
  ...
  const response = await fetch(
    `https://api.github.com/app/installations/${installationId}/access_tokens`,
    {
      ...
      headers: {
        ...
        "User-Agent": "sw-bug-reports-worker",
...
async function createGitHubIssue(
  ...
  const categoryLine = report.category
